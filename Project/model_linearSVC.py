...
ls_parameters = {'penalty': ['l1', 'l2'],
                 'dual': [False],
                 'C': [0.5, 0.7, 1, 5, 10]
                 }

model = GridSearchCV(svm.LinearSVC(), ls_parameters, cv=4, verbose=2)
...
# output
# svc = svm.LinearSVC(C=10, penalty='l1', dual=False, verbose=2)
# svc.fit(train_X, train_Y)
# predict_Y = svc.predict(test_X)
# score = svc.score(train_X, train_Y)
# print("LinearSVC score: Score = %f" % score)
# out = pd.DataFrame(test.loc[:, "Id"], columns=["Id"])
# out.insert(1, "Response", predict_Y)
# out.to_csv("Submission/LinearSVC.csv", index=False)
